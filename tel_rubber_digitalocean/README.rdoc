...
* Deployment instructions
  ...
  1) Setup environment variables in rubber.profile
  ...
  3) Ensure your digital ocean private key is in your authentication agent
    ...
    => ssh-add ~/.ssh/digitalocean/nick
  
  4) Create the droplet
    => RUBBER_ENV=staging ALIAS=staging ROLES=background_worker,common,db:primary=true,nginx,postgresql,postgresql_master,redis,redis_master,sidekiq,unicorn,web,app,whenever cap rubber:create
  ...
  5) Rename the droplet via DigitalOcean UI
    ie: staging-staging becomes <project name>-staging
  ...
  10) Wrap up
    => ssh root@staging.<project name>.com
      => ln -s /mnt/<project name>-staging/current app
      => cd app
